...
classdef table
  ...
  methods
    ...
    ## statistical information on each of its variables. The output is printed
    ## to the Octave console (command window, stdout, or the like in your current
    ## session), in a format suited for human consumption. The output format is
    ## not fixed or formally defined, and may change over time. It is only
    ## suitable for human display, and not for parsing or programmatic use.
    ##
    ## This method supports, to some degree, extension by other packages. If your
    ## Octave session has loaded other packages which supply extension implementaions
    ## of `summary`, Tablicious will use those in preference to its own internal
    ## implementation, and you will get different, and hopefully better, output.
    ##
    ## @end deftypefn
    function out = summary (this)
      # This function lookup is a hook to allow Statistics or other Toolboxes to
      # supply advanced or extended implementations of the table summary logic, which
      # can be switched on at run time without dependence of package load order.
      #
      # This mechanism is currently not used, and the +ofstats package does not actually
      # exist. This is just an example of how one *could* do an extension mechanism like
      # this, where package A defines a method blah on Foo but not a function blah,
      # package B wants to specialize or extend/supplant A's Foo.blah, but can't do so by
      # defining a function blah, because a blah method will take precedence over a blah
      # function, and also you want to do this at run time, with no user configuration needed,
      # and in a manner independent of package load order between A and B.
      #
      # Tablicious makes the assumption that the +ofstats package is owned by Statistics,
      # and if there is an ofstats.summary function, it supports table inputs without need
      # to delegate back to the table.summary method.
      #
      # This function does not do a try/catch fallback to Tablicious' own summary
      # implementation, because my view is that using try/catch like that is gross, and may
      # mask errors in the underlying assumptions of an extension mechanism like this, and
      # it's better to draw them out in the open and deal with them.
      w = which ('ofstats.summary');
      if (! isempty (w))
        # Looks like OF Statistics or something else has supplied an extended implementation
        # of summary() to this Octave session. Prefer that.
        ofstats.summary (this);
      else
        # No extension implementation. Use Tablicious' own implementation.
        tblish.summary (this);
      endif
    endfunction
    ...
    function out = rows2vars (this, varargin)
      ...
      out = table(new_var_values{:}, 'VariableNames', new_var_names);
    ...
    function out = restrict (this, arg)
      ...
      elseif (isnumeric (arg) || islogical (arg))
        ...
        error('restrict: invalid input type: %s', class (arg))
      endif
    endfunction

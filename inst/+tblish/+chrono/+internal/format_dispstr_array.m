...
function out = format_dispstr_array (strs)
  ...
  if ismatrix (strs)
      ...
      page_ixs = tblish.chrono.internal.mycombvec (high_ixs);
      chunks = {};
      for i_page = 1:size (page_ixs, 1)
        page_ix = page_ixs(i_page,:);
        chunks{end+1} = sprintf ('(:,:,%s) = ', ...
          strjoin (tblish.chrono.internal.num2cellstr (page_ix), ':')); %#ok<*AGROW>
...
function out = prettyprint_matrix (strs)
  ...
  widths = max (lens, 1);
  formats = tblish.chrono.internal.sprintfv ('%%%ds', widths);
